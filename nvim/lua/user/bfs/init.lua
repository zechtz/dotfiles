...
M.openOptions = { window = "b %s", vsplit = "vert sb %s", hsplit = "sb %s" }
...
function M.setKeymaps(win, buf)
  ...
  vim.api.nvim_buf_set_keymap(buf, "n", "<CR>", string.format(
    [[:<C-U>lua require'user.bfs'.selBufNum(%s, 'window', vim.v.count)<CR>]],
    win), { nowait = true, noremap = true, silent = true })
  vim.api.nvim_buf_set_keymap(buf, "n", "q", ':lua require"user.bfs".close()<CR>',
    { nowait = true, noremap = true, silent = true })
  vim.api.nvim_buf_set_keymap(buf, "n", "d",
    string.format([[:lua require'user.bfs'.closeBufNum(%s)<CR>]], win),
    { nowait = true, noremap = true, silent = true })
  vim.api.nvim_buf_set_keymap(buf, "n", "l", string.format(
    [[:<C-U>lua require'user.bfs'.selBufNum(%s, 'window', vim.v.count)<CR>]],
    win), { nowait = true, noremap = true, silent = true })
  vim.api.nvim_buf_set_keymap(buf, "n", "s", string.format(
    [[:<C-U>lua require'user.bfs'.selBufNum(%s, 'hsplit', vim.v.count)<CR>]],
    win), { nowait = true, noremap = true, silent = true })
  vim.api.nvim_buf_set_keymap(buf, "n", "v", string.format(
    [[:<C-U>lua require'user.bfs'.selBufNum(%s, 'vsplit', vim.v.count)<CR>]],
    win), { nowait = true, noremap = true, silent = true })
end
...
M.set_buffers = function(buf, current_buf)
  for i, b in ipairs(M.get_bufs()) do
    ...
    if filename:split(" ", true)[1] == "Terminal:" then
      hl_group = hl_group .. "term"
      file_icon, file_icon_color = require("nvim-web-devicons").get_icon_color("terminal",
        extension,
        { default = true })
    end
    ...
    vim.api.nvim_buf_set_text(buf, i - 1, max_width - tostring(linenr):len() + padding, i - 1,
      max_width, { " " .. linenr })
