...
-- Opens all folds with a given fold level. Does not open folds inside the fold recursively.
function M.open_folds_with_level(level)
  local win_view = vim.fn.winsaveview()
  local line_count = vim.api.nvim_buf_line_count(0)
  local line = 1
  ...
  while line <= line_count do
    if vim.fn.foldlevel(line) == level and vim.fn.foldclosed(line) ~= -1 then
      vim.api.nvim_win_set_cursor(0, { line, 0 })
      vim.cmd.normal({ "zo", bang = true }) -- Open the fold at the current line
      local end_line = vim.fn.foldclosedend(line)
      line = end_line > 0 and (end_line + 1) or (line + 1)
    else
      line = line + 1
...
-- Opens all folds with a given treesitter textobject. Does not open folds inside the fold recursively.
function M.open_text_object_folds(textobject)
  local win_view = vim.fn.winsaveview()
  vim.api.nvim_win_set_cursor(0, { 1, 0 })
  while true do
    local cursor = vim.api.nvim_win_get_cursor(0)
...
-- Closes all folds within the current visual selection
function M.close_folds_in_selection()
...
-- Opens all folds within the current visual selection
function M.open_folds_in_selection()
  local _, line_start, _, _ = unpack(vim.fn.getpos("'<"))
  local _, line_end, _, _ = unpack(vim.fn.getpos("'>"))
  local win_view = vim.fn.winsaveview()
